-- =========================================================
-- BLOCO 1 (FASE 2) — MODO CASAL
-- households, household_members, orçamento e lista compartilhados
-- =========================================================
...
CREATE TABLE public.household_members (
  ...
  user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE, -- null até o convite ser aceito
  invited_email TEXT,
  status TEXT NOT NULL DEFAULT 'pending', -- pending | accepted | left
  -- o que o parceiro(a) enxerga da pessoa dentro do modo casal
...
-- Função auxiliar (security definer) para checar se o usuário pertence a um household,
-- evitando recursão infinita nas policies de household_members.
...
-- households: só quem criou ou já é membro aceito pode ver/editar
...
-- household_members: a própria linha, ou membros aceitos do mesmo household
...
-- =========================
-- HOUSEHOLD BUDGET SETTINGS (orçamento compartilhado, opcional)
-- =========================
...
-- =========================
-- SHOPPING LIST ITEMS — passa a suportar itens do lar (compartilhados)
-- =========================
...
-- Substitui a policy antiga por uma que também libera acesso aos itens do household
...
-- =========================
-- Função utilitária: aceitar convite por código
-- =========================
...
BEGIN
  ...
  -- limite de 2 pessoas por household no modo casal
